//! Internal ELF helpers shared by JIT discovery and StackPulse's native loader.
...
pub fn find_section_header<'a>(name: &str, elf: &'a Elf) -> Option<&'a goblin::elf::SectionHeader> {
...
pub fn find_section_range(name: &str, elf: &Elf) -> Option<Range<u64>> {
    let sh = find_section_header(name, elf)?;
    Some(sh.sh_addr..sh.sh_addr.checked_add(sh.sh_size)?)
}
