...
#[cfg(feature = "pe")]
use crate::pe::{DataAtRvaRange, PeUnwinderError, PeUnwinding};
use crate::rule_cache::CacheResult;
use crate::unwind_result::UnwindResult;
use crate::unwind_rule::UnwindRule;
use crate::{FrameAddress, FramePointerFallbackReason, UnwindFrame, UnwindFrameOutcome};
...
/// An unwinder that reports whether each step used module unwind information
/// or frame-pointer fallback.
///
/// This interface has the same recovery behavior as [`Unwinder`]. It adds the
/// reason for a successful frame-pointer fallback to each result.
pub trait UnwinderWithDetails: Unwinder {
    /// Unwind one frame and report the source of the rule that was used.
    fn unwind_frame_with_details<F>(
        ...
        cache: &mut Self::Cache,
        read_stack: &mut F,
    ) -> Result<UnwindFrameOutcome, Error>
    where
        F: FnMut(u64) -> Result<u64, ()>;
}
...
impl<U: UnwinderWithDetails, F: FnMut(u64) -> Result<u64, ()>> UnwindIterator<'_, '_, '_, U, F> {
    /// Yield the next frame together with any frame-pointer fallback reason.
    pub fn next_with_details(&mut self) -> Result<Option<UnwindFrame>, Error> {
        let (next, fallback_reason) = match self.state {
            UnwindIteratorState::Initial(pc) => {
                self.state = UnwindIteratorState::Unwinding(FrameAddress::InstructionPointer(pc));
                return Ok(Some(UnwindFrame::new(
                    FrameAddress::InstructionPointer(pc),
                    None,
                )));
            }
            UnwindIteratorState::Unwinding(address) => {
                let outcome = self.unwinder.unwind_frame_with_details(
                    address,
                    &mut self.regs,
                    self.cache,
                    self.read_stack,
                )?;
                (outcome.return_address(), outcome.fallback_reason())
            }
            UnwindIteratorState::Done => return Ok(None),
        };
        match next {
            Some(return_address) => {
                let return_address = FrameAddress::from_return_address(return_address)
                    .ok_or(Error::ReturnAddressIsNull)?;
                self.state = UnwindIteratorState::Unwinding(return_address);
                Ok(Some(UnwindFrame::new(return_address, fallback_reason)))
            }
            None => {
                self.state = UnwindIteratorState::Done;
                Ok(None)
...
fn fallback_depends_on_sample(error: UnwinderError) -> bool {
    match error {
        UnwinderError::Dwarf(
            DwarfUnwinderError::StackPointerMovedBackwards
            | DwarfUnwinderError::DidNotAdvance
        ...
        #[cfg(feature = "pe")]
        UnwinderError::Pe(PeUnwinderError::MissingStackData(_)) => true,
        _ => false,
    }
}
...
impl<D: Deref<Target = [u8]>, A: Unwinding, P: AllocationPolicy> UnwinderInternal<D, A, P> {
    ...
    where
        F: FnMut(u64) -> Result<u64, ()>,
    {
        self.unwind_frame_with_details(address, regs, cache, read_stack)
            .map(UnwindFrameOutcome::return_address)
    }

    pub fn unwind_frame_with_details<F>(
        &self,
        address: FrameAddress,
        regs: &mut A::UnwindRegs,
        cache: &mut Cache<A::UnwindRule, P>,
