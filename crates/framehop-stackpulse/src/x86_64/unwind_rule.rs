...
mod test {
    ...
    #[test]
    fn uncovered_older_frame_with_zero_bp_ends_stack() {
        ...
        let result = UnwindRuleX86_64::JustReturnIfFirstFrameOtherwiseFp.exec(
            false,
            &mut regs,
            &mut read_stack,
        );
    ...
    #[test]
    fn dwarf_register_defaults_preserve_only_callee_saved_registers() {
        let stack = [0x100200];
        let mut read_stack = |addr| Ok(stack[(addr / 8) as usize]);
        let mut regs = UnwindRegsX86_64::new(0x100300, 0, 0x20);
        regs.set(Reg::RAX, 0xaa);
        ...
        assert_eq!(res, Ok(Some(0x100200)));
        assert_eq!(regs.get_if_set(Reg::RAX), None);
        assert_eq!(regs.get_if_set(Reg::RBX), Some(0xbb));
        assert_eq!(regs.get_if_set(Reg::R12), Some(0xcc));
        assert_eq!(regs.get_if_set(Reg::RSP), Some(8));
        assert_eq!(regs.get_if_set(Reg::RBP), Some(0x20));
    }
}
