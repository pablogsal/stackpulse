        ...
        match options.mode {
            ...
            Mode::Symbolize => {
                ...
                for sample in reader.samples() {
                    ...
                    let resolved = symbolizer.stack_refs_to_cached_frames(
                        sample.process_id,
                        sample.stack_id,
                        raw,
                    );
                    frames += resolved.len();
                    checksum = checksum.wrapping_add(resolved_frame_score(&resolved));
                }
            }
        }
    }
...
fn resolved_frame_score(frames: &[ResolvedFrame]) -> usize {
    frames.iter().fold(0, |score, frame| match frame {
        ResolvedFrame::Python(frame) => score
            .wrapping_add(frame.file_name.len())
        ...
        ResolvedFrame::Native(frame) => {
            ...
            score
                .wrapping_add(frame.pc as usize)
                .wrapping_add(symbol_score)
        }
    })
}
